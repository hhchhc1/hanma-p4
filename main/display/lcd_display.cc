...
LcdDisplay::~LcdDisplay() {
    for (int i = 0; i < 1 + EXTRA_TILE_COUNT; i++) {
        if (page_dots_[i] != nullptr) {
            lv_obj_del(page_dots_[i]);
        }
    }
    if (lock_btn_ != nullptr) {
        lv_obj_del(lock_btn_);
    }
...
void LcdDisplay::CreateLockScreen() {
    lock_screen_ = lv_obj_create(lv_layer_top());
    lv_obj_set_size(lock_screen_, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_bg_color(lock_screen_, lv_color_hex(0x000000), 0);
    lv_obj_set_style_bg_opa(lock_screen_, LV_OPA_COVER, 0);
    ...
    lv_obj_clear_flag(lock_screen_, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);

    lock_time_label_ = lv_label_create(lock_screen_);
    lv_label_set_text(lock_time_label_, "--:--");
    lv_obj_set_style_text_font(lock_time_label_, &lv_font_montserrat_32, 0);
    lv_obj_set_style_text_color(lock_time_label_, lv_color_white(), 0);
    lv_obj_center(lock_time_label_);
    lv_obj_align(lock_time_label_, LV_ALIGN_CENTER, 0, -20);

    lock_hint_label_ = lv_label_create(lock_screen_);
    lv_label_set_text(lock_hint_label_, "Touch to wake up");
    lv_obj_set_style_text_font(lock_hint_label_, fonts_.text_font, 0);
    lv_obj_set_style_text_color(lock_hint_label_, lv_color_hex(0x888888), 0);
    lv_obj_align(lock_hint_label_, LV_ALIGN_CENTER, 0, 30);

    lv_obj_add_event_cb(lock_screen_, [](lv_event_t *e) {
...
void LcdDisplay::EnterLockScreen() {
    if (locked_) return;
    locked_ = true;

    auto bl = Board::GetInstance().GetBacklight();
    if (bl) bl->SetBrightness(10, false);

    time_t now = time(NULL);
    struct tm *tm = localtime(&now);
    if (tm->tm_year >= 2025 - 1900) {
        lv_label_set_text_fmt(lock_time_label_, "%02d:%02d", tm->tm_hour, tm->tm_min);
    }

    lv_obj_remove_flag(lock_screen_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_move_foreground(lock_screen_);
    if (lock_btn_) lv_obj_add_flag(lock_btn_, LV_OBJ_FLAG_HIDDEN);
}

void LcdDisplay::ExitLockScreen() {
    if (!locked_) return;
    locked_ = false;
    ...
    lv_obj_add_flag(lock_screen_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);
    if (lock_btn_) lv_obj_remove_flag(lock_btn_, LV_OBJ_FLAG_HIDDEN);
}

void LcdDisplay::ShowLockScreen() {
    if (locked_) return;
    locked_ = true;
    lv_obj_remove_flag(lock_screen_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);
...
void LcdDisplay::HideLockScreen() {
    if (!locked_) return;
    locked_ = false;
    lv_obj_add_flag(lock_screen_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);
...
void LcdDisplay::SetupUI() {
    ...
    /* TileView for screen switching */
    tileview_ = lv_tileview_create(container_);
    lv_obj_set_width(tileview_, LV_HOR_RES);
    lv_obj_set_flex_grow(tileview_, 1);
    lv_obj_set_style_border_width(tileview_, 0, LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(tileview_, 0, LV_STATE_DEFAULT);
    lv_obj_set_style_radius(tileview_, 0, LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(tileview_, LV_OPA_TRANSP, LV_STATE_DEFAULT);

    /* Tile 0: Chat content */
    lv_obj_t *tile0 = lv_tileview_add_tile(tileview_, 0, 0, LV_DIR_HOR);
    lv_obj_set_style_bg_opa(tile0, LV_OPA_TRANSP, 0);

    content_ = lv_obj_create(tile0);
    lv_obj_set_style_radius(content_, 0, 0);
    lv_obj_set_size(content_, LV_HOR_RES, LV_PCT(100));
    ...
    /* Extra screens (tiles 1-6) */
    extra_screens_create(tileview_);

    /* Page indicator dots */
    lv_coord_t dot_size = 8;
    lv_coord_t dot_gap = 12;
    lv_coord_t total_w = (1 + EXTRA_TILE_COUNT) * dot_size + EXTRA_TILE_COUNT * dot_gap;
    lv_coord_t start_x = (LV_HOR_RES - total_w) / 2;
    lv_coord_t dot_y = LV_VER_RES - 12;

    for (int i = 0; i < 1 + EXTRA_TILE_COUNT; i++) {
        page_dots_[i] = lv_obj_create(lv_layer_sys());
        lv_obj_set_size(page_dots_[i], dot_size, dot_size);
        lv_obj_set_style_radius(page_dots_[i], LV_RADIUS_CIRCLE, LV_STATE_DEFAULT);
        lv_obj_set_style_border_width(page_dots_[i], 0, LV_STATE_DEFAULT);
        lv_obj_set_style_bg_color(page_dots_[i], lv_color_hex(0xFFFFFF), LV_STATE_DEFAULT);
        lv_obj_set_style_bg_opa(page_dots_[i], (i == 0) ? LV_OPA_100 : LV_OPA_30, LV_STATE_DEFAULT);
        lv_obj_set_pos(page_dots_[i], start_x + i * (dot_size + dot_gap), dot_y);
        lv_obj_clear_flag(page_dots_[i], LV_OBJ_FLAG_SCROLLABLE);
        lv_obj_clear_flag(page_dots_[i], LV_OBJ_FLAG_CLICKABLE);
    }

    lv_obj_add_event_cb(tileview_, [](lv_event_t *e) {
        LcdDisplay *self = (LcdDisplay *)lv_event_get_user_data(e);
        lv_obj_t *tv = (lv_obj_t *)lv_event_get_target(e);
        lv_obj_t *act = lv_tileview_get_tile_act(tv);
        lv_coord_t col = lv_obj_get_index(act);
        for (int i = 0; i < 1 + EXTRA_TILE_COUNT; i++) {
            if (self->page_dots_[i]) {
                lv_obj_set_style_bg_opa(self->page_dots_[i], (i == col) ? LV_OPA_100 : LV_OPA_30, LV_STATE_DEFAULT);
            }
        }
        if (self->ec11_focused_ != nullptr) {
            lv_obj_set_style_border_width(self->ec11_focused_, 0, LV_STATE_DEFAULT);
            self->ec11_focused_ = nullptr;
        }
    ...
    /* Lock button */
    lock_btn_ = lv_obj_create(lv_layer_sys());
    lv_obj_set_size(lock_btn_, 36, 36);
    lv_obj_set_style_radius(lock_btn_, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(lock_btn_, lv_color_hex(0x333333), 0);
    lv_obj_set_style_bg_opa(lock_btn_, LV_OPA_60, 0);
    lv_obj_set_style_border_width(lock_btn_, 0, 0);
    lv_obj_set_style_pad_all(lock_btn_, 0, 0);
    lv_obj_clear_flag(lock_btn_, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_align(lock_btn_, LV_ALIGN_BOTTOM_RIGHT, -10, -10);
    auto lock_icon = lv_label_create(lock_btn_);
    lv_label_set_text(lock_icon, FONT_AWESOME_LOCK);
    lv_obj_set_style_text_font(lock_icon, &font_awesome_30_4, 0);
    lv_obj_set_style_text_color(lock_icon, lv_color_white(), 0);
    lv_obj_center(lock_icon);
    lv_obj_add_event_cb(lock_btn_, [](lv_event_t *e) {
        LcdDisplay *self = (LcdDisplay *)lv_event_get_user_data(e);
        self->EnterLockScreen();
    }, LV_EVENT_CLICKED, this);

    /* Default to master page (Tile 1) on boot */
    lv_obj_set_tile_id(tileview_, 1, 0, LV_ANIM_OFF);
}
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Only scroll to message when chat tile (Tile 0) is active
    auto try_scroll = [this](lv_obj_t *obj, lv_anim_enable_t anim) {
        lv_obj_t *act = lv_tileview_get_tile_act(tileview_);
        if (act != nullptr && lv_obj_get_index(act) == 0) {
            lv_obj_scroll_to_view_recursive(obj, anim);
        }
    };
    ...
    if (child_count >= MAX_MESSAGES) {
        ...
        if (last_child != nullptr) {
            try_scroll(last_child, LV_ANIM_OFF);
        }
    }
    ...
    if (strcmp(role, "user") == 0) {
        ...
        try_scroll(container, LV_ANIM_ON);
    } else if (strcmp(role, "system") == 0) {
        // 为系统消息创建全宽容器以确保居中对齐
        lv_obj_t* container = lv_obj_create(content_);
        lv_obj_set_width(container, LV_HOR_RES);
        ...
        try_scroll(msg_bubble, LV_ANIM_ON);
    }
...
void LcdDisplay::HandleEc11Rotate(int direction) {
    DisplayLockGuard lock(this);
    if (locked_ || tileview_ == nullptr) return;

    if (ec11_mode_ == Ec11Mode::VOLUME) {
        auto codec = Board::GetInstance().GetAudioCodec();
        int vol = codec->output_volume() + direction * 5;
        if (vol < 0) vol = 0;
        if (vol > 100) vol = 100;
        codec->SetOutputVolume(vol);
        ShowNotification(Lang::Strings::VOLUME + std::to_string(vol));
        return;
    }

    if (ec11_mode_ == Ec11Mode::BRIGHTNESS) {
        auto bl = Board::GetInstance().GetBacklight();
        if (bl != nullptr) {
            int b = (int)bl->brightness() + direction * 5;
            if (b < 0) b = 0;
            if (b > 100) b = 100;
            bl->SetBrightness((uint8_t)b, true);
            ShowNotification("亮度 " + std::to_string(b));
        }
        return;
    }

    lv_obj_t* active_tile = lv_tileview_get_tile_act(tileview_);
    if (active_tile == nullptr) return;

    std::vector<lv_obj_t*> clickable;
    std::function<void(lv_obj_t*)> scan = [&](lv_obj_t* parent) {
        uint32_t cnt = lv_obj_get_child_cnt(parent);
        for (uint32_t i = 0; i < cnt; i++) {
            lv_obj_t* child = lv_obj_get_child(parent, i);
            if (lv_obj_has_flag(child, LV_OBJ_FLAG_CLICKABLE)) {
                clickable.push_back(child);
            }
            scan(child);
        }
    };
    ...
    if (clickable.empty()) {
        lv_coord_t col = lv_obj_get_index(active_tile);
        lv_coord_t new_col = col + direction;
        if (new_col < 0 || new_col >= 1 + EXTRA_TILE_COUNT) return;
        lv_obj_set_tile_id(tileview_, new_col, 0, LV_ANIM_ON);
        return;
    }

    auto it = std::find(clickable.begin(), clickable.end(), ec11_focused_);
    int idx = (it != clickable.end()) ? (int)(it - clickable.begin()) : -1;

    auto clear_highlight = [](lv_obj_t* obj) {
        lv_obj_set_style_border_width(obj, 0, LV_STATE_DEFAULT);
    };
    auto set_highlight = [](lv_obj_t* obj) {
        lv_obj_set_style_border_width(obj, 3, LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(obj, lv_color_white(), LV_STATE_DEFAULT);
    };

    if (ec11_focused_ != nullptr && std::find(clickable.begin(), clickable.end(), ec11_focused_) != clickable.end()) {
        clear_highlight(ec11_focused_);
    }

    int new_idx = idx + direction;
    if (new_idx < 0 || new_idx >= (int)clickable.size()) {
        ec11_focused_ = nullptr;
        lv_coord_t col = lv_obj_get_index(active_tile);
        lv_coord_t new_col = col + direction;
        if (new_col < 0 || new_col >= 1 + EXTRA_TILE_COUNT) return;
        lv_obj_set_tile_id(tileview_, new_col, 0, LV_ANIM_ON);
    ...
    ec11_focused_ = clickable[new_idx];
    set_highlight(ec11_focused_);
}

void LcdDisplay::HandleEc11KeyEvent(bool is_press) {
    DisplayLockGuard lock(this);
    if (locked_) return;

    if (is_press) {
        ec11_key_press_time_ = esp_timer_get_time();
        return;
    }

    // Key released: determine short vs long press
    uint64_t elapsed_ms = (esp_timer_get_time() - ec11_key_press_time_) / 1000;

    if (elapsed_ms >= 3000) {
        // Long press (3s+): toggle between NAV and adjustment modes
        if (ec11_mode_ == Ec11Mode::NAV) {
            ec11_mode_ = Ec11Mode::VOLUME;
            ClearEc11Focus();
            ShowNotification("音量模式");
        } else {
            ec11_mode_ = Ec11Mode::NAV;
            ShowNotification("页面模式");
        }
        return;
    }
    ...
    // Short press (< 3s)
    if (ec11_mode_ == Ec11Mode::NAV) {
        if (ec11_focused_ != nullptr) {
            lv_obj_send_event(ec11_focused_, LV_EVENT_CLICKED, nullptr);
        } else {
            Application::GetInstance().ToggleChatState();
        }
    } else if (ec11_mode_ == Ec11Mode::VOLUME) {
        ec11_mode_ = Ec11Mode::BRIGHTNESS;
        ShowNotification("亮度模式");
    } else if (ec11_mode_ == Ec11Mode::BRIGHTNESS) {
        ec11_mode_ = Ec11Mode::VOLUME;
        ShowNotification("音量模式");
    }
}

void LcdDisplay::ClearEc11Focus() {
    if (ec11_focused_ != nullptr) {
        lv_obj_set_style_border_width(ec11_focused_, 0, LV_STATE_DEFAULT);
        ec11_focused_ = nullptr;
    }
}
