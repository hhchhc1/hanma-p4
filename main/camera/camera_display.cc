...
// ESP32-P4 PSRAM cache line = 64 bytes
#define PPA_CACHE_LINE_SIZE 64
...
static void camera_task(void *arg)
{
    ...
    s_scaled_buf_size = ALIGN_UP_BY(s_scaled_buf_size, PPA_CACHE_LINE_SIZE);

    for (int i = 0; i < 3; i++) {
        s_canvas_buf[i] = (uint8_t*)heap_caps_aligned_alloc(PPA_CACHE_LINE_SIZE, s_scaled_buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_canvas_buf[i]) {
    ...
    if (s_web_mutex && !s_web_frame) {
        s_web_frame = (uint8_t*)heap_caps_aligned_alloc(PPA_CACHE_LINE_SIZE, s_scaled_buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_web_frame) {
    ...
    if (s_loading_label) {
        ...
        lv_canvas_set_buffer(s_canvas, s_canvas_buf[0], s_scaled_width, s_scaled_height, LV_COLOR_FORMAT_RGB565);
    ...
    while (s_streaming) {
        ...
        esp_cache_msync(fb[buf.index], ALIGN_UP_BY(cam_buf_bytes, PPA_CACHE_LINE_SIZE),
        ...
        if (s_canvas) {
            if (lvgl_port_lock(100)) {
                lv_canvas_set_buffer(s_canvas, s_canvas_buf[write_idx],
                                     s_scaled_width, s_scaled_height, LV_COLOR_FORMAT_RGB565);
