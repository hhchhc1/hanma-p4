        ...
        if (Capture()) {
            if (DetectMotion(gray_buf_, width_, height_)) {
                ...
                if (now - last_speak_time > 10) {
                    last_speak_time = now;
                    const auto& pcm = Lang::Sounds::PCM_NIHAO;
                    size_t sample_count = pcm.second / sizeof(int16_t);
                    ESP_LOGI(TAG, "Play local: 你好主人 (%zu samples)", sample_count);
                    auto* src = reinterpret_cast<const int16_t*>(pcm.first);
                    std::vector<int16_t> amplified(sample_count);
                    for (size_t i = 0; i < sample_count; i++) {
                        int32_t s = src[i] * 4;
                        if (s > 32767) s = 32767;
                        if (s < -32768) s = -32768;
                        amplified[i] = (int16_t)s;
                    }
                    Application::GetInstance().GetAudioService().PlayPCM(
                        amplified.data(), sample_count);
                }
